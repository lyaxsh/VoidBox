...
const FilePreviewPage: React.FC<FilePreviewPageProps> = ({ file, onPageChange, theme }) => {
  console.log('FilePreviewPage file prop:', file);
  ...
  const handleDownload = () => {
    if (!fileInfo) return;
    window.open(getDownloadUrl((file as any).slug || file.id), '_blank');
  };

  // Preview rendering
  let preview: React.ReactNode = null;
  if (fileInfo?.mimetype?.startsWith('image/')) {
    preview = <img src={fileInfo.download_url} alt={fileInfo.name} className={`mx-auto max-h-96 rounded-xl mb-6${theme === 'dark' ? ' bg-white' : ''}`} style={theme === 'dark' ? { backgroundColor: 'white' } : {}} />;
  } else if (fileInfo?.mimetype?.startsWith('video/')) {
    preview = <video src={fileInfo.download_url} controls className={`mx-auto max-h-96 rounded-xl mb-6${theme === 'dark' ? ' bg-white' : ''}`} style={theme === 'dark' ? { backgroundColor: 'white' } : {}} />;
    ...
    preview = (
      <audio controls className={`mx-auto w-full max-w-2xl mb-6${theme === 'dark' ? ' bg-white text-black' : ''}`} style={theme === 'dark' ? { backgroundColor: 'white', color: 'black' } : {}}>
        <source src={fileInfo.download_url} type={fileInfo.mimetype} />
    ...
    preview = (
      <iframe
        ...
        allow="autoplay"
      >
        <p>Your browser does not support PDF preview. <a href={fileInfo.download_url} target="_blank" rel="noopener noreferrer">Download PDF</a></p>
      </iframe>
    );
  ...
  console.log('noteContent:', noteContent, 'fileInfo:', fileInfo);
